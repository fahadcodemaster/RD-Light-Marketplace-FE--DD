...
export async function approveContract(payload, contractAddress, payloadMarket) {
  console.log("***********");
  console.log("MarketNftcontractAddress",payload);
  console.log("contractAddress",payloadMarket);
  console.log("***********");
  const provider = await detectEthereumProvider();
  if (provider !== window.ethereum) {
    window.web3 = new Web3(provider);
  ...
  return new Promise(async (resolve, reject) => {
    ...
    const provider1 = new ethers.providers.Web3Provider(window.ethereum);
    const signer = provider1.getSigner();
    console.log("jis contract main approve() ha", contractAddress);
    console.log("jis contract ko approve krna ha", payload.approved);
    ...
    await contract
      ...
      .then(async (response) => {
        ...
        console.log("contract payload.approved.datapayload.approved.data", payload.approved);
        let contract = new ethers.Contract(
          payload.approved,
          minABI,
          signer
...
export async function mint(payload, contractAddress) {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    await contract
      .mint(payload[0].to, payload[0].tokenId, payload[0].royality, payload[0].uri, {
        gasLimit: 250000,
      })
      .then((res) => {
        res.wait().then(()=>{
